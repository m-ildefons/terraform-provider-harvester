...
func TestCPUTopologyImport(t *testing.T) {
	...
	importerCPU := &VMImporter{VirtualMachine: vm}

	if got := importerCPU.CPUSockets(); got != 2 {
		t.Errorf("CPUSockets() = %d, want 2", got)
	}
	if got := importerCPU.CPUThreads(); got != 4 {
...
func TestDNSImport(t *testing.T) {
	...
	importerDNS := &VMImporter{VirtualMachine: vm}

	if policy := importerDNS.DNSPolicy(); policy != "None" {
		t.Errorf("DNSPolicy() = %q, want %q", policy, "None")
	}

	dc := importerDNS.DNSConfig()
	if len(dc) != 1 {
...
func TestHypervImport(t *testing.T) {
	...
	importerHV := &VMImporter{VirtualMachine: vm}
	hvList := importerHV.Hyperv()
	...
	// Test nil hyperv
	vmNilHV := &kubevirtv1.VirtualMachine{
		Spec: kubevirtv1.VirtualMachineSpec{
			Template: &kubevirtv1.VirtualMachineInstanceTemplateSpec{
				Spec: kubevirtv1.VirtualMachineInstanceSpec{
					Domain: kubevirtv1.DomainSpec{},
	...
	importerNilHV := &VMImporter{VirtualMachine: vmNilHV}
	if got := importerNilHV.Hyperv(); got != nil {
...
func TestHypervPassthroughImport(t *testing.T) {
	...
	importerHVPT := &VMImporter{VirtualMachine: vm}
	if !importerHVPT.HypervPassthrough() {
	...
	importerNilHVPT := &VMImporter{VirtualMachine: vmNil}
	if importerNilHVPT.HypervPassthrough() {
...
func TestClockImport(t *testing.T) {
	...
	importerClock := &VMImporter{VirtualMachine: vm}
	clockList := importerClock.Clock()
	...
	// Test nil clock
	vmNilClock := &kubevirtv1.VirtualMachine{
		Spec: kubevirtv1.VirtualMachineSpec{
			Template: &kubevirtv1.VirtualMachineInstanceTemplateSpec{
				Spec: kubevirtv1.VirtualMachineInstanceSpec{
					Domain: kubevirtv1.DomainSpec{},
	...
	importerNilClock := &VMImporter{VirtualMachine: vmNilClock}
	if got := importerNilClock.Clock(); got != nil {
...
func TestSysprepDiskImport(t *testing.T) {
	...
	importerSysprep := &VMImporter{VirtualMachine: vm}
	disks, _, err := importerSysprep.Volume()
	if err != nil {
		t.Fatalf("Volume() error: %v", err)
	}
	if len(disks) != 2 {
